...
public final class HttpLoggingInterceptor implements Interceptor {
  ...
  public interface Logger {
    void log(Chain chain, String message);

    /** A {@link Logger} defaults output appropriate for the current platform. */
    Logger DEFAULT = new Logger() {
      @Override public void log(Chain chain, String message) {
  ...
  @Override public Response intercept(Chain chain) throws IOException {
    ...
    if (logHeaders) {
      if (hasRequestBody) {
        ...
        if (requestBody.contentType() != null) {
          logger.log(chain, "Content-Type: " + requestBody.contentType());
        }
        if (requestBody.contentLength() != -1) {
          logger.log(chain, "Content-Length: " + requestBody.contentLength());
        }
      }
      ...
      for (int i = 0, count = headers.size(); i < count; i++) {
        ...
        if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
          logger.log(chain, name + ": " + headers.value(i));
        }
      }
    ...
    if (logHeaders) {
      ...
      for (int i = 0, count = headers.size(); i < count; i++) {
        logger.log(chain, headers.name(i) + ": " + headers.value(i));
      }
